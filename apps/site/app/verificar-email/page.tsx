...
type VerificationState = "waiting" | "checking" | "success" | "already_verified" | "invalid";
...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    const sanitizedLocation = new URL(window.location.href);
    sanitizedLocation.searchParams.delete("token");
    window.history.replaceState(
      null,
      "",
      `${sanitizedLocation.pathname}${sanitizedLocation.search}${sanitizedLocation.hash}`,
    );
    ...
    setState("checking");
    void fetch(`${apiUrl}/v1/auth/email-verification/confirm`, {
      method: "POST",
      credentials: "include",
