...
export function profileIdForScope(access: AccessOrganization, unitId: string): ProfileId | null {
  return profileIdsForScope(access, unitId)[0] ?? null;
}

export function profileIdsForScope(access: AccessOrganization, unitId: string): ProfileId[] {
  const result: ProfileId[] = [];
  for (const candidate of rolePriority) {
    const profileId = roleToProfile[candidate];
    if (
      profileId &&
      access.roles.some(
        (binding) =>
