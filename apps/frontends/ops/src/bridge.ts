...
export interface ShellPrintResult {
  success: boolean;
  status: string;
  errorCode?: string;
  printerId?: string;
...
export function shellPrintingAvailable() {
  return typeof window.HybridWebView?.InvokeDotNet === "function";
}

export async function sendShellPrintJob(
  printJob: unknown,
  idempotencyKey: string,
): Promise<ShellPrintResult | null> {
  const invoke = window.HybridWebView?.InvokeDotNet;
  if (!invoke) return null;
  try {
    const result = await invoke<Record<string, unknown>>("SendPrintJobAsync", [
