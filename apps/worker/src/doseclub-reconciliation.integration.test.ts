...
describe("DoseClub reconciliation worker concurrency", () => {
  it("schedules once, claims with SKIP LOCKED and reclaims an expired lease", async (context) => {
    ...
    const databaseName = `gm_t32_worker_${randomUUID().replaceAll("-", "")}`;
    ...
    try {
      ...
      const migrations = (await readdir(migrationsDirectory))
        ...
        .sort();
      assert.equal(migrations.at(-1), "0027_doseclub_reconciliation.sql");
      ...
      await database.client`
        ...
        values (${organizationId}, 'Worker Reconciliation Ltda', 'Worker Reconciliation', '33444555000181')
      `;
      await database.client`
      ...
      await database.client`
        ...
        values (${organizationId}, ${unitId}, 'doseclub', 'active', ${JSON.stringify({ branchId: "worker-unit" })}::jsonb)
      `;
      ...
      >`
        select id, status, finding_count
        from doseclub_reconciliation_runs
        where organization_id = ${organizationId} and unit_id = ${unitId}
      `;
      assert.equal(scheduled.length, 1);
      assert.deepEqual(
        { status: scheduled[0]?.status, findingCount: scheduled[0]?.finding_count },
      ...
      >`
        select status, lease_owner, version
        from doseclub_reconciliation_runs where id = ${expiredRunId}
      `;
