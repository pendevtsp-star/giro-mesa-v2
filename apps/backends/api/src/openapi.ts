...
import { addManagementReportsOpenApi } from "./openapi-management-reports.js";

const outputDirectory = resolve("openapi");
const outputFile = resolve(outputDirectory, "openapi.json");
const { app, document } = await createApplication({ checkDatabaseReadiness: false });
try {
  addManagementReportsOpenApi(document);
