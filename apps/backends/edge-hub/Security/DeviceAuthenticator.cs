...
public sealed class DeviceAuthenticator(IOptions<HubOptions> options, HubStore store)
{
    ...
    public async Task<PairResult> PairAsync(PairDeviceRequest request)
    {
        ...
        if (!Guid.TryParse(request.DeviceId, out var deviceId) || string.IsNullOrWhiteSpace(request.DeviceName))
        {
        ...
        await store.SavePairedDeviceAsync(Hash(token), deviceId.ToString(), request.DeviceName);
        return new(true, token, null, StatusCodes.Status200OK);
    }

    public async Task<AuthenticatedDevice?> AuthenticateAsync(string? token)
    {
        if (string.IsNullOrWhiteSpace(token)) return null;
        var deviceId = await store.GetActiveDeviceIdAsync(Hash(token));
        return deviceId is null ? null : new AuthenticatedDevice(deviceId);
    }

    public async Task<bool> IsAuthorizedAsync(string? token) => await AuthenticateAsync(token) is not null;

    public static bool MatchesDeviceScope(string authenticatedDeviceId, string claimedDeviceId) =>
        string.Equals(authenticatedDeviceId, claimedDeviceId, StringComparison.OrdinalIgnoreCase);
