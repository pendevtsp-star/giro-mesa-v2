...
| Atendimento | `apps/frontends/ops/src/features/salon/SalonPage.tsx`, `FloorPlan.tsx`, seletores operacionais em `salon-operations.ts` | `salon.css`, `operations.shared.tsx`, `operational-dispatch.ts`, geometria em `packages/domain/src/floor-geometry.ts`, API `pilot-operations`, `tests/e2e-live/salon-live.spec.ts` e `docs/runbooks/salon-production-homologation.md` |
