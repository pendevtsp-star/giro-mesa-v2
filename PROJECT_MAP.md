...
| Reserva e divergência de estoque na venda | `apps/backends/api/src/pilot-operations/order-inventory.ts` | Reserva compartilhada por POS e `public-menu/public-order.service.ts`, baixa/estorno em `worker/src/inventory.ts`, confirmação em Ops `inventory-shortage-confirmation.ts` e `features/shell/InventoryShortageDialog.tsx`; ficha opcional, divergência auditada, vínculo/local preservados no envio e alocações de lotes preservadas no consumo |
...
| Visão geral e passagem de turno | `apps/frontends/ops/src/features/dashboard/{DashboardPage,ShiftHandoverPanel}.tsx` | prioridades em `management-overview.service.ts`, snapshot e ciência em `pilot-pos.service.ts`, eventos `audit_events` e contratos `shiftHandover*` |
...
| Balcão/comanda e impressão | `apps/frontends/ops/src/features/counter/CounterPage.tsx`, `CounterWorkspace.tsx` | contas vinculadas em `ServiceAccounts.tsx`/`SplitConsumptionPanel.tsx`, impressão em `account-printing.ts`, API `pilot-pos.*`, vínculo/taxa persistidos na migration `0083_linked_service_accounts.sql`, formatter/gateway do Edge Hub e `docs/runbooks/salon-production-homologation.md` |
...
| Produção KDS e impressoras | `apps/frontends/ops/src/features/kds/KdsPage.tsx`, `KdsSettingsPage.tsx`, `features/device/ProductionPrintersPanel.tsx` | `ProductionPrinterForm.tsx`, `ProductionStationPolicies.tsx`, política de pré-conta em `BillPrintingSettings.tsx` e `production-printing.*`, `kds.navigation.ts`, `api.ts`, contratos/schema operacionais, fila cloud e Edge Hub |
| Cardápio operacional | `apps/frontends/ops/src/features/catalog/CatalogPage.tsx` | `CatalogExperience.tsx`, `components/` (`CatalogInventoryPicker.tsx` e `CatalogInventorySection.tsx`), ficha opcional compartilhada `inventory/RecipeManager.tsx`, `catalog.css`, API `pilot-catalog.*`; projeção de estoque/custo restrita à gestão, criação e vínculo atômicos, migration `0084_inventory_resale_before_catalog.sql` |
...
| Delivery | `apps/frontends/ops/src/features/delivery/DeliveryPage.tsx` | `src/growth.shared.tsx`, `src/realtime.ts`, API `growth/`, diagnóstico `pilot/delivery-projection-status`, `packages/db/src/growth-schema.ts` e migration `0082_delivery_failure_states.sql` |
| Clientes, CRM e WhatsApp | `apps/frontends/ops/src/features/crm/CrmPage.tsx`, `CrmWhatsappWorkspace.tsx` | API `growth/` e `evolution-go.ts`; histórico paginado `customers/:customerId/history`, entregas paginadas `campaigns/:campaignId/deliveries`; realtime `src/realtime.ts`, worker `whatsapp.ts`/`outbox.ts`, schema/mídia privada em `packages/db/src/growth-schema.ts` e `whatsapp-artifacts.ts`, migration `0074_crm_operational_inbox.sql`, `docs/runbooks/evolution-go.md` |
| Estoque e compras | `apps/frontends/ops/src/features/inventory/InventoryPage.tsx`, `InventoryControls.tsx`, `features/purchases/PurchasesPage.tsx` | `InventoryWorkspace.tsx`, salvamento e reconciliação de vasilhames em `inventory-item-save.ts`, fila `inventory-offline.ts`, API `management.inventory-controls.*`, worker `inventory.ts`, `packages/db/src/management-schema.ts` e [revenda e retornáveis](docs/runbooks/catalog-inventory.md) |
...
| Configurações do estabelecimento | `apps/frontends/ops/src/features/settings/SettingsPage.tsx`, `SetupChecklist.tsx`, `settings.ts` e `settings.css` | Ops `api.ts`/shell, API `organizations/establishment-settings.service.ts` (preparação, revisão, cópia, histórico, restauração e conferência manual por canal em `settings/channel-checks`, com evidência/ator/data em auditoria), mídia do catálogo e `packages/contracts`/`packages/domain` |
...
| Caixa e documentos financeiros | `apps/frontends/ops/src/features/cash/{CashPage,CashClosureReceipt,CashHistoryPanel}.tsx`, `features/finance/FinancePage.tsx` | `management.service.ts`, liquidação compartilhada por `management-settlements.service.ts`, mídia privada `fiscal-artifacts.ts` e migration `0081_management_finance_documents_and_settlement_payments.sql` |
...
| Publicação/QR do Cardápio | `apps/backends/api/src/public-menu/`, `pilot-catalog.service.ts` | Preços/disponibilidade atuais dos produtos publicados, confirmação transacional de total em `public-order-total.ts`, Customer `components/menu-experience.tsx` e `lib/order-price-change.ts`, sessão curta da mesa, tokens e mídia pública; regressão em `tests/customer-pricing.config.ts` |
...
- `tests/e2e/customer-tracking.spec.ts`: acompanhamento QR, alergia, recuperação após falha/reload e 375/1440 px; API controlada em `tests/customer-tracking.config.ts`.
