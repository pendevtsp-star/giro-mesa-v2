...
test(
  "round-trips PostgreSQL, objects and encrypted configuration before a functional smoke",
  ...
  () => {
    ...
    try {
      ...
      assert.equal(readFileSync(join(objectRestore, "menus", "cover.txt"), "utf8"), "cover-asset-v1\n");
      assert.equal(
        readFileSync(join(objectRestore, "receipts", "receipt.json"), "utf8"),
        '{"amountCents":4200}\n',
      );
      assert.deepEqual(readFileSync(join(configRestore, "configuration.enc")), readFileSync(configSource));

      const evidence = JSON.parse(readFileSync(evidencePath, "utf8"));
      assert.equal(evidence.artifact, artifact);
      assert.equal(evidence.migrationId, "0026_doseclub_integration");
